    ...
    try {
        ...
        document.querySelectorAll(".sell-energy-btn").forEach(btn => {
            btn.addEventListener("click", (e) => {
                ...
                let estimatedEarnings = (delegateAmount / remaining) * proratedCft;
                console.log(`Order ${orderId}: Initial estimated earnings ${estimatedEarnings.toFixed(2)} CFT (delegate: ${delegateAmount}, prorated CFT: ${proratedCft})`);
                ...
                document.getElementById("delegate-amount").addEventListener("input", () => {
                    ...
                    estimatedEarnings = (newDelegateAmount / remaining) * proratedCft;
                    console.log(`Order ${orderId}: Updated earnings to ${estimatedEarnings.toFixed(2)} CFT (new delegate: ${newDelegateAmount}, prorated CFT: ${proratedCft})`);
                    document.getElementById("estimated-earnings").textContent = `${estimatedEarnings.toFixed(2)} CFT`;
                });
            });
        });
