...
const tokenDetails = {
  cft: {
    ...
    stakingAddress: 'TDuzExgC7J4zdYeeV52Lskn9WAJJRChmhS', // Replace with actual staking contract address
    decimals: 6,
    displayName: 'CFT',
...
async function setPoolSize(token) {
  let processingModal = null;
  try {
    if (!isValidTronAddress(tokenDetails[token].stakingAddress)) throw new Error(`Invalid staking address: ${tokenDetails[token].stakingAddress}`);
    if (!userAddress || !isValidTronAddress(userAddress)) throw new Error('Invalid user address. Please reconnect wallet.');
    const poolSize = document.getElementById('pool-size').value;
    if (!poolSize || isNaN(poolSize) || Number(poolSize) <= 0) throw new Error('Please enter a valid pool size.');
    const { availableEnergy, shortfall, requiredEnergy } = await checkUserEnergy(userAddress, token, 'setPoolSize');
    if (shortfall > 0) {
      const totalRequired = shortfall + SAFETY_ENERGY;
      const hasEnoughEnergy = await checkDelegatorEnergy(totalRequired);
      if (hasEnoughEnergy) {
    ...
    processingModal = showProcessingModal('(2/2)');
    const stakingContract = stakingContracts[token];
    if (!stakingContract || !stakingContract.methods.setPoolSize) throw new Error(`Staking contract for ${token} not properly initialized`);
    const owner = await stakingContract.methods.owner().call();
    if (userAddress !== owner) throw new Error('Only the owner can set the pool size.');
    const amount = BigInt(poolSize) * BigInt(10 ** tokenDetails[token].rewardDecimals);
    console.log('Sending set pool size transaction...');
    ...
    if (!tx.result || !tx.transaction) throw new Error('Failed to create set pool size transaction');
    const signedTx = await tronWeb.trx.sign(tx.transaction);
    const broadcastTx = await tronWeb.trx.sendRawTransaction(signedTx);
    if (!broadcastTx.result) throw new Error('Failed to broadcast set pool size transaction');
    console.log('Set pool size transaction broadcasted:', broadcastTx.txid);
    hideProcessingModal(processingModal);
  } catch (error) {
    hideProcessingModal(processingModal);
    console.error(`Error setting pool size for ${token}:`, error);
    alert(`Error setting pool size: ${error.message}.`);
  }
}

async function addToPool(token) {
  let processingModal = null;
  try {
    if (!isValidTronAddress(tokenDetails[token].stakingAddress)) throw new Error(`Invalid staking address: ${tokenDetails[token].stakingAddress}`);
    if (!userAddress || !isValidTronAddress(userAddress)) throw new Error('Invalid user address. Please reconnect wallet.');
    const amount = document.getElementById('add-to-pool').value;
    if (!amount || isNaN(amount) || Number(amount) <= 0) throw new Error('Please enter a valid amount to add.');
    const { availableEnergy, shortfall, requiredEnergy } = await checkUserEnergy(userAddress, token, 'addToPool');
    if (shortfall > 0) {
      const totalRequired = shortfall + SAFETY_ENERGY;
      const hasEnoughEnergy = await checkDelegatorEnergy(totalRequired);
      if (hasEnoughEnergy) {
    ...
    processingModal = showProcessingModal('(2/2)');
    const stakingContract = stakingContracts[token];
    if (!stakingContract || !stakingContract.methods.addToPool) throw new Error(`Staking contract for ${token} not properly initialized`);
    const owner = await stakingContract.methods.owner().call();
    if (userAddress !== owner) throw new Error('Only the owner can add to the pool.');
    const amountInSun = BigInt(amount) * BigInt(10 ** tokenDetails[token].rewardDecimals);
    console.log('Sending add to pool transaction...');
    const tx = await tronWeb.transactionBuilder.triggerSmartContract(
      tokenDetails[token].stakingAddress,
      'addToPool()',
      { callValue: amountInSun.toString() },
      [],
      userAddress
    );
    if (!tx.result || !tx.transaction) throw new Error('Failed to create add to pool transaction');
    const signedTx = await tronWeb.trx.sign(tx.transaction);
    const broadcastTx = await tronWeb.trx.sendRawTransaction(signedTx);
    if (!broadcastTx.result) throw new Error('Failed to broadcast add to pool transaction');
    console.log('Add to pool transaction broadcasted:', broadcastTx.txid);
    hideProcessingModal(processingModal);
  } catch (error) {
    hideProcessingModal(processingModal);
    console.error(`Error adding to pool for ${token}:`, error);
    alert(`Error adding to pool: ${error.message}.`);
  }
}

async function setDailyPayoutPercentage(token) {
  let processingModal = null;
  try {
    if (!isValidTronAddress(tokenDetails[token].stakingAddress)) throw new Error(`Invalid staking address: ${tokenDetails[token].stakingAddress}`);
    if (!userAddress || !isValidTronAddress(userAddress)) throw new Error('Invalid user address. Please reconnect wallet.');
    const percentage = document.getElementById('daily-payout-percentage').value;
    if (!percentage || isNaN(percentage) || Number(percentage) <= 0 || Number(percentage) > 10000) throw new Error('Please enter a valid percentage (1-10000).');
    const { availableEnergy, shortfall, requiredEnergy } = await checkUserEnergy(userAddress, token, 'setDailyPayout');
    if (shortfall > 0) {
      const totalRequired = shortfall + SAFETY_ENERGY;
      const hasEnoughEnergy = await checkDelegatorEnergy(totalRequired);
      if (hasEnoughEnergy) {
    ...
    processingModal = showProcessingModal('(2/2)');
    const stakingContract = stakingContracts[token];
    if (!stakingContract || !stakingContract.methods.setDailyPayoutPercentage) throw new Error(`Staking contract for ${token} not properly initialized`);
    const owner = await stakingContract.methods.owner().call();
    if (userAddress !== owner) throw new Error('Only the owner can set the daily payout percentage.');
    console.log('Sending set daily payout percentage transaction...');
    const tx = await tronWeb.transactionBuilder.triggerSmartContract(
      tokenDetails[token].stakingAddress,
      'setDailyPayoutPercentage(uint256)',
      {},
      [{ type: 'uint256', value: percentage }],
      userAddress
    );
    if (!tx.result || !tx.transaction) throw new Error('Failed to create set daily payout percentage transaction');
    const signedTx = await tronWeb.trx.sign(tx.transaction);
    const broadcastTx = await tronWeb.trx.sendRawTransaction(signedTx);
    if (!broadcastTx.result) throw new Error('Failed to broadcast set daily payout percentage transaction');
    console.log('Set daily payout percentage transaction broadcasted:', broadcastTx.txid);
    hideProcessingModal(processingModal);
  } catch (error) {
    hideProcessingModal(processingModal);
    console.error(`Error setting daily payout percentage for ${token}:`, error);
    alert(`Error setting daily payout percentage: ${error.message}.`);
  }
}

async function setClaimTimeout(token) {
  let processingModal = null;
  try {
    if (!isValidTronAddress(tokenDetails[token].stakingAddress)) throw new Error(`Invalid staking address: ${tokenDetails[token].stakingAddress}`);
    if (!userAddress || !isValidTronAddress(userAddress)) throw new Error('Invalid user address. Please reconnect wallet.');
    const timeoutDays = document.getElementById('claim-timeout').value;
    if (!timeoutDays || isNaN(timeoutDays) || Number(timeoutDays) <= 0) throw new Error('Please enter a valid number of days.');
    const { availableEnergy, shortfall, requiredEnergy } = await checkUserEnergy(userAddress, token, 'setClaimTimeout');
    if (shortfall > 0) {
      const totalRequired = shortfall + SAFETY_ENERGY;
      const hasEnoughEnergy = await checkDelegatorEnergy(totalRequired);
      if (hasEnoughEnergy) {
    ...
    processingModal = showProcessingModal('(2/2)');
    const stakingContract = stakingContracts[token];
    if (!stakingContract || !stakingContract.methods.setClaimTimeout) throw new Error(`Staking contract for ${token} not properly initialized`);
    const owner = await stakingContract.methods.owner().call();
    if (userAddress !== owner) throw new Error('Only the owner can set the claim timeout.');
    const timeoutSeconds = BigInt(timeoutDays) * BigInt(86400); // Convert days to seconds
    console.log('Sending set claim timeout transaction...');
    const tx = await tronWeb.transactionBuilder.triggerSmartContract(
      tokenDetails[token].stakingAddress,
      'setClaimTimeout(uint256)',
      {},
      [{ type: 'uint256', value: timeoutSeconds.toString() }],
      userAddress
    );
    if (!tx.result || !tx.transaction) throw new Error('Failed to create set claim timeout transaction');
    const signedTx = await tronWeb.trx.sign(tx.transaction);
    const broadcastTx = await tronWeb.trx.sendRawTransaction(signedTx);
    if (!broadcastTx.result) throw new Error('Failed to broadcast set claim timeout transaction');
    console.log('Set claim timeout transaction broadcasted:', broadcastTx.txid);
    hideProcessingModal(processingModal);
  } catch (error) {
    hideProcessingModal(processingModal);
    console.error(`Error setting claim timeout for ${token}:`, error);
    alert(`Error setting claim timeout: ${error.message}.`);
  }
}

async function setAuthorizedWallet(token) {
  let processingModal = null;
  try {
    if (!isValidTronAddress(tokenDetails[token].stakingAddress)) throw new Error(`Invalid staking address: ${tokenDetails[token].stakingAddress}`);
    if (!userAddress || !isValidTronAddress(userAddress)) throw new Error('Invalid user address. Please reconnect wallet.');
    const wallet = document.getElementById('authorized-wallet').value;
    if (!isValidTronAddress(wallet)) throw new Error('Please enter a valid TRON address.');
    const { availableEnergy, shortfall, requiredEnergy } = await checkUserEnergy(userAddress, token, 'setAuthorizedWallet');
    if (shortfall > 0) {
      const totalRequired = shortfall + SAFETY_ENERGY;
      const hasEnoughEnergy = await checkDelegatorEnergy(totalRequired);
      if (hasEnoughEnergy) {
    ...
    processingModal = showProcessingModal('(2/2)');
    const stakingContract = stakingContracts[token];
    if (!stakingContract || !stakingContract.methods.setAuthorizedWallet) throw new Error(`Staking contract for ${token} not properly initialized`);
    const owner = await stakingContract.methods.owner().call();
    if (userAddress !== owner) throw new Error('Only the owner can set the authorized wallet.');
    console.log('Sending set authorized wallet transaction...');
    const tx = await tronWeb.transactionBuilder.triggerSmartContract(
      tokenDetails[token].stakingAddress,
    ...
    if (!tx.result || !tx.transaction) throw new Error('Failed to create set authorized wallet transaction');
    const signedTx = await tronWeb.trx.sign(tx.transaction);
    const broadcastTx = await tronWeb.trx.sendRawTransaction(signedTx);
    if (!broadcastTx.result) throw new Error('Failed to broadcast set authorized wallet transaction');
    console.log('Set authorized wallet transaction broadcasted:', broadcastTx.txid);
    hideProcessingModal(processingModal);
  } catch (error) {
    hideProcessingModal(processingModal);
    console.error(`Error setting authorized wallet for ${token}:`, error);
    alert(`Error setting authorized wallet: ${error.message}.`);
  }
}

async function withdrawTRX(token) {
  let processingModal = null;
  try {
    if (!isValidTronAddress(tokenDetails[token].stakingAddress)) throw new Error(`Invalid staking address: ${tokenDetails[token].stakingAddress}`);
    if (!userAddress || !isValidTronAddress(userAddress)) throw new Error('Invalid user address. Please reconnect wallet.');
    const amount = document.getElementById('withdraw-trx-amount').value;
    const recipient = document.getElementById('withdraw-trx-recipient').value;
    if (!amount || isNaN(amount) || Number(amount) <= 0) throw new Error('Please enter a valid TRX amount.');
    if (!isValidTronAddress(recipient)) throw new Error('Please enter a valid recipient address.');
    const { availableEnergy, shortfall, requiredEnergy } = await checkUserEnergy(userAddress, token, 'withdrawTRX');
    if (shortfall > 0) {
      const totalRequired = shortfall + SAFETY_ENERGY;
      const hasEnoughEnergy = await checkDelegatorEnergy(totalRequired);
      if (hasEnoughEnergy) {
    ...
    processingModal = showProcessingModal('(2/2)');
    const stakingContract = stakingContracts[token];
    if (!stakingContract || !stakingContract.methods.withdrawTRX) throw new Error(`Staking contract for ${token} not properly initialized`);
    const owner = await stakingContract.methods.owner().call();
    if (userAddress !== owner) throw new Error('Only the owner can withdraw TRX.');
    const amountInSun = BigInt(amount) * BigInt(10 ** tokenDetails[token].rewardDecimals);
    console.log('Sending withdraw TRX transaction...');
    const tx = await tronWeb.transactionBuilder.triggerSmartContract(
      tokenDetails[token].stakingAddress,
      'withdrawTRX(uint256,address)',
      {},
      [
    ...
    if (!tx.result || !tx.transaction) throw new Error('Failed to create withdraw TRX transaction');
    const signedTx = await tronWeb.trx.sign(tx.transaction);
    const broadcastTx = await tronWeb.trx.sendRawTransaction(signedTx);
    if (!broadcastTx.result) throw new Error('Failed to broadcast withdraw TRX transaction');
    console.log('Withdraw TRX transaction broadcasted:', broadcastTx.txid);
    hideProcessingModal(processingModal);
  } catch (error) {
    hideProcessingModal(processingModal);
    console.error(`Error withdrawing TRX for ${token}:`, error);
    alert(`Error withdrawing TRX: ${error.message}.`);
  }
}

async function withdrawTRC20(token) {
  let processingModal = null;
  try {
    if (!isValidTronAddress(tokenDetails[token].stakingAddress)) throw new Error(`Invalid staking address: ${tokenDetails[token].stakingAddress}`);
    if (!userAddress || !isValidTronAddress(userAddress)) throw new Error('Invalid user address. Please reconnect wallet.');
    const amount = document.getElementById('withdraw-trc20-amount').value;
    const recipient = document.getElementById('withdraw-trc20-recipient').value;
    if (!amount || isNaN(amount) || Number(amount) <= 0) throw new Error('Please enter a valid CFT amount.');
    if (!isValidTronAddress(recipient)) throw new Error('Please enter a valid recipient address.');
    const { availableEnergy, shortfall, requiredEnergy } = await checkUserEnergy(userAddress, token, 'withdrawTRC20');
    if (shortfall > 0) {
      const totalRequired = shortfall + SAFETY_ENERGY;
      const hasEnoughEnergy = await checkDelegatorEnergy(totalRequired);
      if (hasEnoughEnergy) {
    ...
    processingModal = showProcessingModal('(2/2)');
    const stakingContract = stakingContracts[token];
    if (!stakingContract || !stakingContract.methods.withdrawTRC20) throw new Error(`Staking contract for ${token} not properly initialized`);
    const owner = await stakingContract.methods.owner().call();
    if (userAddress !== owner) throw new Error('Only the owner can withdraw TRC20 tokens.');
    const amountInUnits = BigInt(amount) * BigInt(10 ** tokenDetails[token].decimals);
    console.log('Sending withdraw TRC20 transaction...');
    const tx = await tronWeb.transactionBuilder.triggerSmartContract(
      tokenDetails[token].stakingAddress,
      'withdrawTRC20(uint256,address)',
      {},
      [
    ...
    if (!tx.result || !tx.transaction) throw new Error('Failed to create withdraw TRC20 transaction');
    const signedTx = await tronWeb.trx.sign(tx.transaction);
    const broadcastTx = await tronWeb.trx.sendRawTransaction(signedTx);
    if (!broadcastTx.result) throw new Error('Failed to broadcast withdraw TRC20 transaction');
    console.log('Withdraw TRC20 transaction broadcasted:', broadcastTx.txid);
    hideProcessingModal(processingModal);
  } catch (error) {
    hideProcessingModal(processingModal);
    console.error(`Error withdrawing TRC20 for ${token}:`, error);
    alert(`Error withdrawing TRC20: ${error.message}.`);
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const setPoolSizeButton = document.getElementById('set-pool-size-button');
  if (setPoolSizeButton) {
    setPoolSizeButton.addEventListener('click', async (e) => {
      e.preventDefault();
      await setPoolSize(key);
    });
  } else {
    console.error('Set pool size button not found');
  }

  const addToPoolButton = document.getElementById('add-to-pool-button');
  if (addToPoolButton) {
    addToPoolButton.addEventListener('click', async (e) => {
      e.preventDefault();
      await addToPool(key);
    });
  } else {
    console.error('Add to pool button not found');
  }

  const setDailyPayoutButton = document.getElementById('set-daily-payout-button');
  if (setDailyPayoutButton) {
    setDailyPayoutButton.addEventListener('click', async (e) => {
      e.preventDefault();
      await setDailyPayoutPercentage(key);
    });
  } else {
    console.error('Set daily payout button not found');
  }

  const setClaimTimeoutButton = document.getElementById('set-claim-timeout-button');
  if (setClaimTimeoutButton) {
    setClaimTimeoutButton.addEventListener('click', async (e) => {
      e.preventDefault();
      await setClaimTimeout(key);
    });
  } else {
    console.error('Set claim timeout button not found');
  }

  const setAuthorizedWalletButton = document.getElementById('set-authorized-wallet-button');
  if (setAuthorizedWalletButton) {
    setAuthorizedWalletButton.addEventListener('click', async (e) => {
      e.preventDefault();
      await setAuthorizedWallet(key);
    });
  } else {
    console.error('Set authorized wallet button not found');
  }

  const withdrawTRXButton = document.getElementById('withdraw-trx-button');
  if (withdrawTRXButton) {
    withdrawTRXButton.addEventListener('click', async (e) => {
      e.preventDefault();
      await withdrawTRX(key);
    });
  } else {
    console.error('Withdraw TRX button not found');
  }

  const withdrawTRC20Button = document.getElementById('withdraw-trc20-button');
  if (withdrawTRC20Button) {
    withdrawTRC20Button.addEventListener('click', async (e) => {
      e.preventDefault();
      await withdrawTRC20(key);
    });
  } else {
    console.error('Withdraw TRC20 button not found');
  }
