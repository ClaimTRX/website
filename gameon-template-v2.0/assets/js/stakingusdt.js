...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('stake-button-token3').addEventListener('click', () => stakeTokens('token3'));
    document.getElementById('unstake-button-token3').addEventListener('click', () => unstakeTokens('token3'));
    document.getElementById('claim-rewards-button-token3').addEventListener('click', () => claimRewards('token3'));
});
...
async function updateUI() {
    await updateAvailableTokens('token3');
    await updateStakedAmount('token3');
    await updateEstimatedAPR('token3');
    await updateClaimableRewards('token3');
    await updateTotalClaimedRewards('token3');
}
...
async function updateEstimatedAPR(token) {
    ...
    const apr = stakedAmountCFT > 0 ? (projectedRewards / (stakedAmountCFT * priceCFT) * 100).toFixed(2) + '%' : 'N/A';
...
async function updateClaimableRewards(token) {
    ...
    document.getElementById(`claimable-rewards-${token}`).innerText = formatNumber(tronWeb.fromSun(claimableRewardsRaw)) + ' USDT';
}
