...
const tokenDetails = {
  cft: {
    ...
    stakingAddress: 'TAbu6yKiVRbs3c7tcwFnreupEfVW9t8d9K',
    decimals: 6,
    displayName: 'CFT',
...
function showRewardWarningModal(rewardUnits) {
  return new Promise((resolve, reject) => {
    ...
    if (rewardDisplay) {
      rewardDisplay.textContent = `${rewardUnits.toFixed(2)} CFT`;
    } else {
...
async function updateTopBarUI(token, first = false, userData) {
  ...
  if (cacheData && Date.now() - cacheData.timestamp < CACHE_TIMEOUT_MS && !first) {
    ...
    updateElement(`user-total-claimed-cft`, fmtCft(cacheData.data.userTotalClaimed), `user-total-claimed-cft`);
    return;
  }
  try {
    if (first) {
    ...
    const totalClaimed = toUnits(userTotalClaimedRaw, d.rewardDecimals);
    // ROI: (totalClaimed / stakedAmount) * 100 since both in CFT
    const roiPct = (stakedAmount > 0 && userData.isActive) ? (totalClaimed / stakedAmount) * 100 : 0;
    ...
    // APY calculation: (yourNextPayout * 365 * 100) / stakedAmount (1:1 since same token)
    let apyPct = (stakedAmount > 0 && userData.isActive && yourNextPayout > 0)
      ? (yourNextPayout * 365 * 100) / stakedAmount
      : 0;
    ...
    updateElement(`user-total-claimed-cft`, fmtCft(userTotalClaimed), `user-total-claimed-cft`);
  } catch (e) {
...
async function updateStatsGridUI(token, first = false, userData) {
  ...
  try {
    ...
    updateElement('pool-size', fmtCft(poolSize), 'pool-size');
    updateElement('daily-payout', '1.00% / day', 'daily-payout');
    updateElement('your-next-payout', fmtCft(yourNextPayout), 'your-next-payout');
  } catch (e) {
...
function updateClaimTimer(timeoutSec, lastClaimTs, isActive, isWhitelisted, initialRewards = '0', initialBalance = '0') {
  ...
  const tick = async () => {
    ...
    if (Date.now() - cacheTimestamp >= CACHE_TIMEOUT_MS) {
      try {
        pendingRewards = await retryWithBackoff(() => readStakingContracts['cft'].methods.earned(userAddress).call());
      ...
      contractBalanceRaw = await retryWithBackoff(() => readTokenContracts['cft'].methods.balanceOf(tokenDetails['cft'].stakingAddress).call().catch(() => '0'));
...
async function claimRewards(token) {
  ...
  const run = async () => {
    try {
      ...
      const contractBalanceRaw = await retryWithBackoff(() => readTokenContracts[token].methods.balanceOf(tokenDetails[token].stakingAddress).call());
